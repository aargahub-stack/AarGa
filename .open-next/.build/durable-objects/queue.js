...
// C:/Users/Aravindh/AppData/Local/npm-cache/_npx/b8f71965aba33be8/node_modules/@opennextjs/cloudflare/dist/api/durable-objects/queue.js
...
var DOQueueHandler = class extends DurableObject {
  ...
  async addToFailedState(msg) {
    ...
    if (!this.disableSQLite) {
      this.sql.exec("INSERT OR REPLACE INTO failed_state (id, data, buildId) VALUES (?, ?, ?)", msg.MessageDeduplicationId, JSON.stringify(updatedFailedState), "KH-UmzipxDRQVlO5EBdl4");
    }
    await this.addAlarm();
  }
  async addAlarm() {
  ...
  async initState() {
    ...
    this.sql.exec("DELETE FROM failed_state WHERE buildId != ?", "KH-UmzipxDRQVlO5EBdl4");
    this.sql.exec("DELETE FROM sync WHERE buildId != ?", "KH-UmzipxDRQVlO5EBdl4");
